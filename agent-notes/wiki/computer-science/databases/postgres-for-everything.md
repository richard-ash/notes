...
|---|---|---|
| Elasticsearch / Solr | `tsvector`/`tsquery` with GIN indexes (planner and schema tuning in [[postgres-full-text-search]]); `pg_textsearch` (BM25) or ParadeDB's `pg_search` (Tantivy-backed) when ranking or scale outgrows built-in FTS | Built-in ranking is weaker than BM25; the extensions are young relative to core Postgres and not universally available on managed hosts. For short-string matching see [[postgres-trigram-search]] instead. |
| MongoDB | JSONB columns with GIN indexes; Contentful, Instacart, and The Guardian are cited as production migrations | JSONB is, as Bachrach puts it in [[postgres-fundamentals]], a sharp knife: schemaless columns forfeit constraints and make bad data easy. Use it for genuinely variable shapes, not as an excuse to skip modelling. |
