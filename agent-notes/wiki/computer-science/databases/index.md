...
- [[sql-json-serialization]] — Eder's case for emitting nested JSON/XML from SQL (correlated subqueries + FOR JSON PATH / json_agg) instead of an entity→DTO→Jackson mapping layer; why correlated subqueries avoid both N+1 and cartesian joins, the Postgres form (json_* not jsonb_* for output, coalesce for empty arrays), the caveats Eder skips (serialization CPU lands on the primary, per-row SubPlans, types leave the type system, coupling relocates), and temporal notes on jOOQ MULTISET, PG16/17 SQL/JSON, and PostgREST/Hasura as the thesis industrialised
